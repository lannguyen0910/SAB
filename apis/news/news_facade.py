...
class NewsFacade(object):
    ...
    def send_messages(self, articles, name="News", channel='news', n=8):
        ...
        for ind, articles in enumerate(final_articles):
            if ind == 0:
                message = self.create_rich_message_layout(
                    name, articles, cont=False)
            else:
                message = self.create_rich_message_layout(
                    name, articles, cont=True)
            slack_response = self.emit(message, channel)
            logging.info(
                f"Sent message {ind+1}/{message_count} to Slack:\n{slack_response}")
